                                  ...
                                  const sensor_msgs::msg::PointCloud2::ConstSharedPtr laserCloud2)
{
  ...
  odometryIn.header.frame_id = robot_name+"/map";
  odometryIn.child_frame_id = robot_name+"/sensor_at_scan";
  pubOdometryPointer->publish(odometryIn);

  transformToMap.frame_id_ = robot_name+"/map";
  transformTfGeom = tf2::toMsg(transformToMap);
  transformTfGeom.header.stamp = laserCloud2->header.stamp;
  transformTfGeom.child_frame_id = robot_name+"/sensor_at_scan";
  ...
  scan_data.header.frame_id = robot_name+"/sensor_at_scan";
...
int main(int argc, char** argv)
{
  ...
  subOdometry.subscribe(nh, "/"+robot_name+"/state_estimation", qos_profile);
  subLaserCloud.subscribe(nh, "/"+robot_name+"/registered_scan", qos_profile);
  ...
  pubOdometryPointer = nh->create_publisher<nav_msgs::msg::Odometry>("/"+robot_name+"/state_estimation_at_scan", 5);

  tfBroadcasterPointer = std::make_unique<tf2_ros::TransformBroadcaster>(*nh);

  pubLaserCloud = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/sensor_scan", 2);
