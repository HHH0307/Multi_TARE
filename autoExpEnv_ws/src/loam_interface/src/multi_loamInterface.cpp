...
public:
  ...
  : Node("loamInterface")
  {
    ...
    this->declare_parameter<string>("robot_name", robot_name);
    ...
    pubLaserCloud = this->create_publisher<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/registered_scan", 5);
    pubOdometry = this->create_publisher<nav_msgs::msg::Odometry>("/"+robot_name+"/state_estimation", 5);
...
private:
  ...
  void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odom) const
  {
    ...
    odomData.child_frame_id = robot_name+"/sensor";
    ...
    if (sendTF) {
      if (!reverseTF) {
        ...
        transformTfGeom.header.frame_id = robot_name+"/sensor";
