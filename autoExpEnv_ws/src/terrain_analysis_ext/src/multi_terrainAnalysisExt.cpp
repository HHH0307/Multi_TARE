...
int main(int argc, char** argv)
{
  ...
  auto subClearing = nh->create_subscription<std_msgs::msg::Float32>("/"+robot_name+"/cloud_clearing", 5, clearingHandler);

  auto subTerrainCloudLocal = nh->create_subscription<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/terrain_map", 2, terrainCloudLocalHandler);

  auto pubTerrainCloud = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/terrain_map_ext", 2);
  ...
  while (status)
  {
    ...
    if (newlaserCloud)
    {
      ...
      terrainCloud2.header.frame_id = robot_name+"/map";
