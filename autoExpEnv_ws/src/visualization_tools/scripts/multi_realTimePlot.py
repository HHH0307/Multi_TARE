...
class Listener(Node):
    ...
    def __init__(self):
        ...
        self.fig3.set_ylabel(f'{self.robot_name}'+"_Algorithm\nRuntime (s)", fontsize=12)
        self.fig3.set_xlabel(f'{self.robot_name}'+"_Time Duration (s)", fontsize=12) #only set once
        ...
        self.time_duration_subscription = self.create_subscription(
            Float32,
            f'/{self.robot_name}/time_duration',
        ...
        self.explored_volume_subscription = self.create_subscription(
            Float32,
            f'/{self.robot_name}/explored_volume',
        ...
        self.traveling_distance_subscription = self.create_subscription(
            Float32,
            f'/{self.robot_name}/traveling_distance',
    ...
    def plot_callback(self):
        ...
        if self.count >= 100:
            ...
            self.fig.canvas.draw()
            self.fig.canvas.flush_events()  # 仅处理事件，不激活窗口
