...
void odometryHandler(const nav_msgs::msg::Odometry::ConstSharedPtr odom)
{
  ...
  trajectory2.header.frame_id = robot_name+"/map";
...
void laserCloudHandler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr laserCloudIn)
{
  ...
  *exploredAreaCloud += *laserCloud;
  ...
  if (exploredAreaDisplayCount >= 5 * exploredAreaDisplayInterval) {
    ...
    exploredArea2.header.frame_id = robot_name+"/map";
...
int main(int argc, char** argv)
{
  ...
  auto subRuntime = nh->create_subscription<std_msgs::msg::Float32>("/"+robot_name+"/runtime", 5, runtimeHandler);

  auto pubOverallMap = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/overall_map", 5);

  pubExploredAreaPtr = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/explored_areas", 5);

  pubTrajectoryPtr = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/trajectory", 5);
  
  pubExploredVolumePtr = nh->create_publisher<std_msgs::msg::Float32>("/"+robot_name+"/explored_volume", 5);

  pubTravelingDisPtr = nh->create_publisher<std_msgs::msg::Float32>("/"+robot_name+"/traveling_distance", 5);

  pubTimeDurationPtr = nh->create_publisher<std_msgs::msg::Float32>("/"+robot_name+"/time_duration", 5);
  ...
  metricFile += "_" + robot_name + "_" + timeString + ".txt";
  trajFile += robot_name + "_" + timeString + ".txt";
  ...
  while (status) {
    ...
    if (overallMapDisplayCount >= 100 * overallMapDisplayInterval) {
      ...
      overallMap2.header.frame_id = robot_name+"/map";
