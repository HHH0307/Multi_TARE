...
int main(int argc, char** argv)
{
  ...
  auto subOdom = nh->create_subscription<nav_msgs::msg::Odometry>("/"+robot_name+"/state_estimation", 5, odomHandler);

  auto subPath = nh->create_subscription<nav_msgs::msg::Path>("/"+robot_name+"/path", 5, pathHandler);
  ...
  auto subStop = nh->create_subscription<std_msgs::msg::Int8>("/"+robot_name+"/stop", 5, stopHandler);

  auto pubSpeed = nh->create_publisher<geometry_msgs::msg::TwistStamped>("/"+robot_name+"/cmd_vel", 5);

  auto pubSpeed_robot_1 = nh->create_publisher<geometry_msgs::msg::Twist>("/"+robot_name+"/cmd_vel"+"1", 5); // 新增

  geometry_msgs::msg::TwistStamped cmd_vel;
  geometry_msgs::msg::Twist cmd_vel_robot_1; // 新增
  cmd_vel.header.frame_id = robot_name+"/vehicle";
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      // std::cout << "路径长度: " << pathSize << std::endl; // 新增
      ...
      // std::cout << "理想 速度: " << joySpeed3 << std::endl; // 新增
      ...
      if (pubSkipCount < 0) {
        ...
        if (fabs(vehicleSpeed) <= maxAccel / 100.0)
        {
          ...
          // std::cout << "速度小于速度分辨率, 速度为0" << std::endl; // 新增
        }
        else cmd_vel.twist.linear.x = vehicleSpeed;
        if(vehicleSpeed == 0)
        {
          // std::cout << " 常规计算, 速度为0" << std::endl; // 新增
        }
        cmd_vel.twist.angular.z = vehicleYawRate;    
        cmd_vel_robot_1.angular = cmd_vel.twist.angular; // 新增  
        cmd_vel_robot_1.linear = cmd_vel.twist.linear; // 新增
        pubSpeed->publish(cmd_vel);
        pubSpeed_robot_1->publish(cmd_vel_robot_1); // 新增
