...
//新增转换
geometry_msgs::msg::Quaternion map_to_robot;
double robot_X;
double robot_Y;
double robot_Z;
...
void odometryHandler(const nav_msgs::msg::Odometry::ConstSharedPtr odom)
{
  ...
  //新增获取
  map_to_robot = odom->pose.pose.orientation;
  robot_X=odom->pose.pose.position.x - cos(yaw) * sensorOffsetX + sin(yaw) * sensorOffsetY;
  robot_Y=odom->pose.pose.position.y - sin(yaw) * sensorOffsetX - cos(yaw) * sensorOffsetY;
...
// hqy 新增
void TransToMap(nav_msgs::msg::Path& path_map, 
                                          const nav_msgs::msg::Path& path_vehicle, 
                                          const geometry_msgs::msg::Quaternion& map_to_robot_, 
                                          const double& robot_X_, 
                                          const double& robot_Y_, 
                                          const double& robot_Z_)
{
    // 创建从map到robot的变换
    tf2::Quaternion m_t_r(map_to_robot_.x, map_to_robot_.y, map_to_robot_.z, map_to_robot_.w);
    tf2::Transform t_map_to_robot(m_t_r, tf2::Vector3(robot_X_, robot_Y_, robot_Z_));

    for(int i = 0; i < path_vehicle.poses.size(); i++)
    {
      tf2::Vector3 temp_point(
          path_vehicle.poses[i].pose.position.x,
          path_vehicle.poses[i].pose.position.y,
          path_vehicle.poses[i].pose.position.z
      );
      tf2::Vector3 result = t_map_to_robot(temp_point);
      geometry_msgs::msg::PoseStamped temp;
      temp.pose.position.x = result.m_floats[0];
      temp.pose.position.y = result.m_floats[1];
      temp.pose.position.z = result.m_floats[2];
      path_map.poses.push_back(temp);
    }
}
...
int main(int argc, char** argv)
{
  ...
  nh->declare_parameter<string>("robot_name", robot_name);
  ...
  auto subOdometry = nh->create_subscription<nav_msgs::msg::Odometry>("/"+robot_name+"/state_estimation", 5, odometryHandler);

  auto subLaserCloud = nh->create_subscription<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/registered_scan", 5, laserCloudHandler);

  auto subTerrainCloud = nh->create_subscription<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/terrain_map", 5, terrainCloudHandler);

  auto subJoystick = nh->create_subscription<sensor_msgs::msg::Joy>("/"+robot_name+"/joy", 5, joystickHandler);

  auto subGoal = nh->create_subscription<geometry_msgs::msg::PointStamped> ("/"+robot_name+"/way_point", 5, goalHandler);

  auto subSpeed = nh->create_subscription<std_msgs::msg::Float32>("/"+robot_name+"/speed", 5, speedHandler);

  auto subBoundary = nh->create_subscription<geometry_msgs::msg::PolygonStamped>("/"+robot_name+"/navigation_boundary", 5, boundaryHandler);

  auto subAddedObstacles = nh->create_subscription<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/added_obstacles", 5, addedObstaclesHandler);

  auto subCheckObstacle = nh->create_subscription<std_msgs::msg::Bool>("/"+robot_name+"/check_obstacle", 5, checkObstacleHandler);

  auto pubPath = nh->create_publisher<nav_msgs::msg::Path>("/"+robot_name+"/path", 5);
  auto pubPath_map = nh->create_publisher<nav_msgs::msg::Path>("/"+robot_name+"/path_map", 5);
  nav_msgs::msg::Path path;

  #if PLOTPATHSET == 1
  auto pubFreePaths = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/"+robot_name+"/free_paths", 2);
  #endif
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        if (selectedGroupID >= 0) {
          ...
          path.header.frame_id = robot_name+"/vehicle";
          pubPath->publish(path);
          // 添加转到全局坐标下的路径点
          nav_msgs::msg::Path path_map;
          TransToMap(path_map, path, map_to_robot, robot_X, robot_Y, robot_Z);
          path_map.header = path.header;
          path_map.header.frame_id = robot_name + "/map";
          pubPath_map->publish(path_map);
          ...
          freePaths2.header.frame_id = robot_name+"/vehicle";
