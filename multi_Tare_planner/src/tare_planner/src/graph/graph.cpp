...
void Graph::SetNodePosition(int node_index, const Eigen::Vector3d& position)
{
  ...
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("standalone_logger"), "Graph::SetNodePosition: node_index: " << node_index << " not in range [0, "
                                                               << positions_.size() - 1 << "]");
  }
}
...
void Graph::AddOneWayEdge(int from_node_index, int to_node_index, double distance)
{
  ...
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("standalone_logger"), "Graph::AddOneWayEdge: from_node_index: " << from_node_index << " to_node_index: "
