...
// 保留之前的SoveGlobal TSP
exploration_path_ns::ExplorationPath GridWorld::SolveGlobalTSP(const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>& viewpoint_manager, std::vector<int>& ordered_cell_indices, const std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)
{
  ...
  if (exploring_cell_indices.empty())
  {
    ...
    else
    {
      ...
      {
        // RCLCPP_ERROR_STREAM(rclcpp::get_logger("standalone_logger"), "Cannot find path home " << sub.transpose());
      }
    }
    return global_path;
  }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//求解 全局  TSP 输入  视点管理，有序单元索引，关键位姿图    返回一个全局路径     探索路径或者是返回路径
//通过  维护同一个   keypose_graph  来求解mtsp 问题
exploration_path_ns::ExplorationPath GridWorld::SolveGlobalTSP_(
    const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>& viewpoint_manager,
    std::vector<int>& ordered_cell_indices, std::vector<double>& M_TSP_subgrapher_code,
    std::map<int, std::vector<double>>& MTSP_subgrapher_map_,
    const std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)  //添加编码  传到外部去发送
{
  /****** Get the node on keypose graph associated with the robot position    获取与机器人位置相关联的keypose图上的节点
   * *****/
  double min_dist_to_robot = DBL_MAX;
  ...
  //需要对所有的Cell  分类，改成多人的就是    1 .获取其他机器人位置；2.并对探索单元进行共享；3.更新各自探索单元的状态

  /****** Get all the connected exploring cells   获取所有连接的  探测单元  *****/
  exploration_path_ns::ExplorationPath global_path;  //全局路径  表示  一个类，里面有容器包含定义的结点
  std::vector<geometry_msgs::msg::Point> exploring_cell_positions;        //探索  单元_本地   位置数据
  std::vector<geometry_msgs::msg::Point> exploring_cell_positions_world;  //探索  单元_世界  位置数据
  ...
  {
    if (subspaces_local_->GetCell(i).GetStatus() == CellStatus::EXPLORING)  //如果本地     是探索状态的  单元   且能到达
                                                                            //放入探索单元集里面
    {
      if (std::find(neighbor_cell_indices_.begin(), neighbor_cell_indices_.end(), i) == neighbor_cell_indices_.end() ||
          (subspaces_->GetCell(i).GetViewPointIndices().empty() &&
           subspaces_->GetCell(i).GetVisitCount() > 1))  //  i  不在邻接单元里面的探索点，或者  没有视点的
      {
            ...
            keypose_graph->GetNodeNum() == 0)  //不用关键位姿图 或 关键位姿图才  初始化
        {
          ...
          exploring_cell_positions.push_back(GetCellPosition(i));  //将当前单位放入探索单元集
          exploring_cell_indices.push_back(i);
        }
        else
        {
          ...
          else  //当不可通行时寻找此单元其他关键位姿结点 与当前    connection_point_geo   最近的点
          {
            ...
            {
              geometry_msgs::msg::Point node_position =
                  keypose_graph->GetNodePosition(node_ind);  //单元内  存在于图上的结点集
              double dist = misc_utils_ns::PointXYZDist<geometry_msgs::msg::Point, geometry_msgs::msg::Point>(
                  node_position, connection_point_geo);
              if (dist < min_dist)
              {
          ...
          if (reachable)
          {
            exploring_cell_positions.push_back(connection_point_geo);  //得到探索单元的位置信息
            exploring_cell_indices.push_back(i);
          }
        }
      }
  ...
  for (int i = 0; i < subspaces_->GetCellNumber(); i++)  //对于子空间的单元
  {
    // std::vector<int>::iterator ret = std::find(neighbor_cell_indices_.begin(), neighbor_cell_indices_.end(),  i);
    // if (subspaces_->GetCell(i).GetStatus() == CellStatus::EXPLORING || (subspaces_local_->GetCell(i).GetStatus() ==
    // CellStatus::EXPLORING && ret!=neighbor_cell_indices_.end() ))
    if (subspaces_->GetCell(i).GetStatus() == CellStatus::EXPLORING)  //如果是全局的     是探索状态的  单元   且能到达
                                                                      //放入探索单元集里面
    {
      if (std::find(neighbor_cell_indices_.begin(), neighbor_cell_indices_.end(), i) == neighbor_cell_indices_.end() ||
          (subspaces_->GetCell(i).GetViewPointIndices().empty() &&
           subspaces_->GetCell(i).GetVisitCount() > 1))  //  i  不在邻接单元里面
      {
        if (!use_keypose_graph_ || keypose_graph == nullptr ||
            keypose_graph->GetNodeNum() == 0)  //不用关键位姿图 或 关键位姿图才  初始化
        {
  ...
  // 添加      本地邻接范围内是否有可到达的探索点，个数。
  num_local_neighbor_cell_reachable = 0;
  for (int i = 0; i < neighbor_cell_indices_.size(); i++)
  {
    int cell_id = neighbor_cell_indices_[i];
    if (subspaces_local_->GetCell(cell_id).GetStatus() == CellStatus::EXPLORING)
    {
      if (!use_keypose_graph_ || keypose_graph == nullptr ||
          keypose_graph->GetNodeNum() == 0)  //不用关键位姿图 或 关键位姿图才  初始化
      {
        // Use straight line connection  使用直线连接
        num_local_neighbor_cell_reachable++;
      }
      else
      {
        Eigen::Vector3d connection_point =
            subspaces_->GetCell(cell_id).GetRoadmapConnectionPoint();  //子空间的路线图连接点
                                                                       //就是距离cell中心最近的视点的值
        geometry_msgs::msg::Point connection_point_geo;
        connection_point_geo.x = connection_point.x();
        connection_point_geo.y = connection_point.y();
  ...
  //
  num_world_neighbor_cell_reachable = 0;
  for (int i = 0; i < neighbor_cell_indices_.size(); i++)
  {
    int cell_id = neighbor_cell_indices_[i];
    if (subspaces_->GetCell(cell_id).GetStatus() == CellStatus::EXPLORING)  //共享覆盖   //共享探索和覆盖
    {
      if (!use_keypose_graph_ || keypose_graph == nullptr ||
          keypose_graph->GetNodeNum() == 0)  //不用关键位姿图 或 关键位姿图才  初始化
      {
        // Use straight line connection  使用直线连接
        num_world_neighbor_cell_reachable++;
      }
      else
      {
        Eigen::Vector3d connection_point =
  ...
  // Put the current robot position in the end  将当前机器人的位置放在最后    需要探索的单元集最后一位？
  exploring_cell_positions.push_back(global_path_robot_position);
  exploring_cell_indices.push_back(-1);  //得到当前本地   可到达（探索点）   边界图

  /******* Construct the distance matrix   构造距离矩阵*****/
  std::vector<std::vector<int>> distance_matrix(exploring_cell_positions.size(),
                                                std::vector<int>(exploring_cell_positions.size(), 0));
  for (int i = 0; i < exploring_cell_positions.size(); i++)
  {
    for (int j = 0; j < i; j++)
    {
      ...
      else
      {
        ...
        distance_matrix[i][j] = static_cast<int>(
            10 * keypose_graph->GetShortestPath(exploring_cell_positions[i], exploring_cell_positions[j], false,
  ...
  exploring_cell_indices_world.push_back(-1);  //得到当前本地   可到达（探索点）   边界图
    ...
    for (int j = 0; j < i; j++)
    {
      ...
      else
      {
        ...
        distance_matrix_world[i][j] = static_cast<int>(
            10 * keypose_graph->GetShortestPath(exploring_cell_positions_world[i], exploring_cell_positions_world[j],
                                                false, path_tmp, false));  //在 关键位姿图上   找到最短距离
      }
    }
  }
  //对角化
  ...
  M_TSP_subgrapher_code = GetMtspSubgrapherCode(exploring_cell_positions, distance_matrix);  //返回个数  1+3*n+n*n

  std::vector<std::vector<int>> distance_matrix_MTSP;
  std::vector<geometry_msgs::msg::Point> exploring_cell_positions_MTSP;  //探索  单元_世界  位置数据 用于判定探索结束与否）
  std::vector<int> exploring_cell_indices_MTSP;  //探索单元  ID（即哪些单元是作为探索的目标，本地全局可到达探索点）
  std::vector<int> robot_position_id_on_exploring_cell_positions_MTSP;  // 机器人位置  在 exploring_cell_positions_MTSP
  ...
  std::cout << std::endl
            << "\033[1;32m"
            << "全局可达到探索点数                              " << exploring_cell_indices_world.size() - 1
            << "\033[0m" << std::endl;
  std::cout << std::endl
            << "\033[1;32m"
            << "本地可达到探索点数                              " << exploring_cell_indices.size() - 1 << "\033[0m"
            << std::endl;
  std::cout << std::endl
            << "\033[1;32m"
            << "本地邻接单元集内可达到探索点数   " << num_local_neighbor_cell_reachable << "\033[0m" << std::endl;
  std::cout << std::endl
            << "\033[1;32m"
            << "全局邻接单元集内可达到探索点数   " << num_world_neighbor_cell_reachable << "\033[0m" << std::endl;

  //检测当前位置的状态
  if (subspaces_world_->GetCell(cur_robot_cell_ind_).GetStatus() == CellStatus::UNSEEN)
  {
    std::cout << "\033[1;32m"
              << " 当前状态：  UNSEEN   "
              << "\033[0m" << std::endl;
    is_fuse_ = false;  //当时不可见或是  探索状态     就得重新变为探索状态
  }
  else if (subspaces_world_->GetCell(cur_robot_cell_ind_).GetStatus() == CellStatus::EXPLORING)
  {
    std::cout << "\033[1;32m"
              << " 当前状态：  EXPLORING   "
              << "\033[0m" << std::endl;
  ...
  else if (subspaces_world_->GetCell(cur_robot_cell_ind_).GetStatus() == CellStatus::COVERED)
  {
    std::cout << "\033[1;32m"
              << " 当前状态：  COVERED  "
              << "\033[0m" << std::endl;
  }
  else if (subspaces_world_->GetCell(cur_robot_cell_ind_).GetStatus() == CellStatus::COVERED_BY_OTHERS)
  {
    std::cout << "\033[1;32m"
              << " 当前状态：  COVERED_BY_OTHERS   "
              << "\033[0m" << std::endl;
  }
  else if (subspaces_world_->GetCell(cur_robot_cell_ind_).GetStatus() == CellStatus::NOGO)
  {
    std::cout << "\033[1;32m"
              << " 当前状态：  NOGO   "
              << "\033[0m" << std::endl;
  ...
  if (is_fuse_)  
  {
    ...
    if (last_path.GetLength() > 0 && exploring_cell_indices_world.size() == 1)  
    {
      int last_goal_cell_id = last_path.nodes_[1].global_subspace_index_;
      if (subspaces_world_->GetCell(last_goal_cell_id).GetStatus() ==
          CellStatus::EXPLORING) 
      {
        std::cout << std::endl
                  << "\033[1;32m"
                  << "之前的目标点  单元ID" << last_goal_cell_id << "\033[0m" << std::endl;
        std::cout << std::endl
                  << "\033[1;32m"
                  << "采用  之前的目标点"
                  << "\033[0m" << std::endl;
    ...
    std::cout << std::endl
              << "\033[1;32m"
              << "目标不是探索状态，切换状态，重置探索逻辑"
              << "\033[0m" << std::endl;
  }
  //切换到  到达目标状态
  if (exploring_cell_indices_world.size() == 1 &&
      num_world_neighbor_cell_reachable == 0) 
  {
    is_fuse_ = GetGlobalMtspGrapher_FusedGrapher(
        exploring_cell_indices, exploring_cell_positions, distance_matrix, exploring_cell_indices_MTSP,
        exploring_cell_positions_MTSP, distance_matrix_MTSP, robot_position_id_on_exploring_cell_positions_MTSP,
        MTSP_subgrapher_map_, keypose_graph, Fuse_Grapher_);
  }

  if (exploring_cell_indices_world.size() == 1 && !is_fuse_)  //融合失败 且 没有可到达的点  就算探索完毕
  {
    return_home_ = true;
  ...
  if (!is_fuse_)  //融合失败  做      TSP    用全局可到达      exploring_cell_positions_world
  {
    ...
    ordered_cell_indices.clear();
    //  打印输出  查看
    // std::cout << std::endl << "\033[1;32m" << "机器人："<<"做 TSP" << "\033[0m" << std::endl;
  }
  else
  {  //成功做 MTSP
    int robot_num = robot_position_id_on_exploring_cell_positions_MTSP.size();
    tsp_solver_ns::DataModel data_model;                //数据模型
    data_model.distance_matrix = distance_matrix_MTSP;  //距离矩阵
    data_model.num_vehicles = robot_num;
    int num = robot_position_id_on_exploring_cell_positions_MTSP.size();
    // robot_position_id_on_exploring_cell_positions_MTSP[0] 是当前机器人 在  exploring_cell_positions_MTSP  上的index
    for (int i = 0; i < num; i++)
    {
      int id = robot_position_id_on_exploring_cell_positions_MTSP[i];
      std::pair<int, int> id_(id, id);
    ...
    ordered_cell_indices.clear();
    //更新 变量方便后续处理
    exploring_cell_indices = exploring_cell_indices_MTSP;
    exploring_cell_positions = exploring_cell_positions_MTSP;
    distance_matrix = distance_matrix_MTSP;
    //打印输出  查看
    std::cout << std::endl
              << "\033[1;32m"
              << "做 MTSP"
  ...
  {
    //做  MTSP打印了
    robot_cell_id = exploring_cell_indices_MTSP[robot_position_id_on_exploring_cell_positions_MTSP.front()];
    std::cout << std::endl
              << "\033[1;32m"
  ...
  if (is_fuse_)  //  融合成功走这个路线     选一个做目标点  传出
  {
    ...
    {
      //  探索补偿
      int robot_num = robot_position_id_on_exploring_cell_positions_MTSP.size();  //机器人数量
      int exploring_num = distance_matrix_MTSP.size() - robot_position_id_on_exploring_cell_positions_MTSP.size();
      if (node_index.size() <= 2 && exploring_num > 0)  // MTSP       没有给本地机器人分配目标点
                                                        // 但还是存在可达到的未探索位置     修正  探索点  给入
      {
        std::vector<int> distance_vector = distance_matrix_MTSP[robot_position_id_on_exploring_cell_positions_MTSP[0]];
        ...
        for (int i = 0; i < distance_vector.size(); i++)
        {
          ...
          {
            if (i == robot_position_id_on_exploring_cell_positions_MTSP[j])  //排除  机器人位置
            {
              flag = true;
            }
          }
          if (!flag)
          {
            goal_position.push_back(exploring_cell_positions_MTSP[i]);
            goal_position_distance.push_back(distance_vector[i]);
          }
        }

        int minPosition = std::min_element(distance_vector.begin(), distance_vector.end()) -
                          distance_vector.begin();  //得到最小点的索引
        //加入本地
        ...
        //加入目标
        exploration_path_ns::Node node_goal(Eigen::Vector3d(goal_position[minPosition].x, goal_position[minPosition].y,
                                                            goal_position[minPosition].z));  //当前位置
        node_goal.type_ = exploration_path_ns::NodeType::GLOBAL_VIEWPOINT;
        node_goal.global_subspace_index_ =
        ...
        //补偿加入     组成3个点
        global_path.Append(global_path.nodes_[0]);
        ordered_cell_indices.push_back(node_goal.global_subspace_index_);

        //打印     到达目标逻辑 的最终结果
        std::cout << "\033[1;32m"
                  << "  做MTSP  最终修正目标点  Cell_id ：    " << node_goal.global_subspace_index_ << "\033[0m"
                  << std::endl;
      ...
      node_local.global_subspace_index_ =
          GetCellInd(exploring_cell_positions[robot_ind].x, exploring_cell_positions[robot_ind].y,
                     exploring_cell_positions[robot_ind].z);
      global_path.Append(node_local);
      int goal_ind = node_index[1];
      exploration_path_ns::Node node_goal(Eigen::Vector3d(exploring_cell_positions[goal_ind].x,
                                                          exploring_cell_positions[goal_ind].y,
                                                          exploring_cell_positions[goal_ind].z));  //当前位置
      node_goal.type_ = exploration_path_ns::NodeType::GLOBAL_VIEWPOINT;
      node_goal.global_subspace_index_ =
          GetCellInd(exploring_cell_positions[goal_ind].x, exploring_cell_positions[goal_ind].y,
                     exploring_cell_positions[goal_ind].z);
      global_path.Append(node_goal);
      global_path.Append(global_path.nodes_[0]);
      ordered_cell_indices.push_back(node_goal.global_subspace_index_);
      std::cout << "\033[1;32m"
                << "  做MTSP  正常目标点  Cell_id ：    " << node_goal.global_subspace_index_ << "\033[0m" << std::endl;
      last_global_goal_path_ = global_path;
      return global_path;
    }
  }
...
GridWorld::SolveGlobalTSP_pro(const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>& viewpoint_manager,
                              std::vector<int>& ordered_cell_indices, std::vector<double>& M_TSP_subgrapher_code,
                              std::map<int, std::vector<double>>& MTSP_subgrapher_map_, bool& is_global_tsp,
                              std::vector<rclcpp::Client<tare_planner::srv::RequestPath>::SharedPtr>& request_path_client_list_, // 替换为 ROS2
                              std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)  //添加编码 传到外部去发送
{
  double min_dist_to_robot = DBL_MAX;
  geometry_msgs::msg::Point global_path_robot_position = robot_position_;  
  Eigen::Vector3d eigen_robot_position(robot_position_.x, robot_position_.y,
  ...
  std::vector<int> exploring_cell_indices;  //探索单元  ID（即哪些单元是作为探索的目标   本地可到达探索点）
  std::vector<int> exploring_cell_indices_world;  //探索单元  ID（即哪些单元是作为探索的目标，本地全局可到达探索点）
  std::vector<int> exploring_cell_ID;                   
  for (int i = 0; i < subspaces_->GetCellNumber(); i++)  
  {
    if (subspaces_local_->GetCell(i).GetStatus() == CellStatus::EXPLORING)  
    {
           ...
           subspaces_->GetCell(i).GetVisitCount() > 1))  
      {
        ...
        else
        {
          ...
          else 
          {
            ...
            {
              geometry_msgs::msg::Point node_position = keypose_graph->GetNodePosition(node_ind);
              double dist = misc_utils_ns::PointXYZDist<geometry_msgs::msg::Point, geometry_msgs::msg::Point>(node_position, connection_point_geo);
              if (dist < min_dist)
              {
                min_dist = dist;
                min_dist_node_ind = node_ind;
    ...
    if (subspaces_->GetCell(i).GetStatus() == CellStatus::EXPLORING)
    {
           ...
           subspaces_->GetCell(i).GetVisitCount() > 1))  
      {
        if (!use_keypose_graph_ || keypose_graph == nullptr ||
            keypose_graph->GetNodeNum() == 0)  
        {
          exploring_cell_positions_world.push_back(GetCellPosition(i)); 
          exploring_cell_indices_world.push_back(i);
        }
        else
        {
          ...
          if (reachable)
          {
            exploring_cell_positions_world.push_back(connection_point_geo); 
            exploring_cell_indices_world.push_back(i);
          }
        }
      }
  ...
  {
    int cell_id = neighbor_cell_indices_[i];
    if (subspaces_->GetCell(cell_id).GetStatus() == CellStatus::EXPLORING) 
    {
      if (!use_keypose_graph_ || keypose_graph == nullptr ||
          keypose_graph->GetNodeNum() == 0) 
      {
  ...
  exploring_cell_positions.push_back(global_path_robot_position);
  exploring_cell_indices.push_back(-1);
  std::vector<std::vector<int>> distance_matrix(exploring_cell_positions.size(), std::vector<int>(exploring_cell_positions.size(), 0));
  for (int i = 0; i < exploring_cell_positions.size(); i++)
  {
    for (int j = 0; j < i; j++)
    {
      ...
      else
      {
        nav_msgs::msg::Path path_tmp;
        distance_matrix[i][j] = static_cast<int>(
            10 * keypose_graph->GetShortestPath(exploring_cell_positions[i], exploring_cell_positions[j], false, path_tmp, false)); 
      }
    }
  }
  for (int i = 0; i < exploring_cell_positions.size(); i++)
  ...
  exploring_cell_positions_world.push_back(global_path_robot_position);
  exploring_cell_indices_world.push_back(-1);  
  ...
  for (int i = 0; i < exploring_cell_positions_world.size(); i++)
  {
    for (int j = 0; j < i; j++)
    {
      ...
      {
        nav_msgs::msg::Path path_tmp;
        distance_matrix_world[i][j] = static_cast<int>(10 * keypose_graph->GetShortestPath(exploring_cell_positions_world[i], exploring_cell_positions_world[j], false, path_tmp, false)); 
      }
    }
  }
  for (int i = 0; i < exploring_cell_positions_world.size(); i++)
  ...
  M_TSP_subgrapher_code = GetMtspSubgrapherCode(exploring_cell_positions, distance_matrix);

  if (robot_statu_ == RobotStatus::Exploring && is_global_tsp == false)
  {
  ...
  std::vector<std::vector<int>> distance_matrix_MTSP;
  std::vector<geometry_msgs::msg::Point> exploring_cell_positions_MTSP;
  std::vector<int> exploring_cell_indices_MTSP;  
  ...
  if (is_fuse_ && robot_statu_ == RobotStatus::Request)  
  {
    exploration_path_ns::ExplorationPath last_path = last_global_goal_path_;
    geometry_msgs::msg::Point goal_point;
    goal_point.x = last_path.nodes_.back().position_[0];
    goal_point.y = last_path.nodes_.back().position_[1];
    goal_point.z = last_path.nodes_.back().position_[2];
    // std::cout << "请求状态的  目标点 " << goal_point << std::endl;
    bool flage_add_edge = false;
    if (last_path.GetLength() > 0)
    {
      int last_goal_cell_id = GetCellInd(goal_point.x, goal_point.y, goal_point.z);
      // std::cout << "请求状态的  目标点的  cell_index    " << last_goal_cell_id << std::endl;
      if (subspaces_world_->GetCell(last_goal_cell_id).GetStatus() ==
          CellStatus::EXPLORING) 
      {
        // std::cout << "目标点   还是探索状态" << std::endl;
        double distance_robot_goal = last_path.GetLength();

        bool is_reachable = false;
        if (keypose_graph->IsPositionReachable(goal_point))
        {
          is_reachable = true;
        }
        else 
        {
          double min_dist = DBL_MAX;
          double min_dist_node_ind = -1;
          for (const auto& node_ind : subspaces_->GetCell(last_goal_cell_id).GetGraphNodeIndices())
          {
            geometry_msgs::msg::Point node_position = keypose_graph->GetNodePosition(node_ind);  //单元内  存在于图上的结点集
            double dist =
                misc_utils_ns::PointXYZDist<geometry_msgs::msg::Point, geometry_msgs::msg::Point>(node_position, goal_point);
            if (dist < min_dist)
            {
              min_dist = dist;
              min_dist_node_ind = node_ind;
          ...
          {
            is_reachable = true;
            goal_point = keypose_graph->GetNodePosition(min_dist_node_ind);
          }
        }
        if (is_reachable) 
        {
          nav_msgs::msg::Path this_path;
          keypose_graph->GetShortestPath(global_path_robot_position, goal_point, true, this_path, false);
          if (this_path.poses.size() >= 2)
          {
            global_path.FromPath(this_path);
            global_path.nodes_.front().type_ = exploration_path_ns::NodeType::ROBOT;
            ...
            global_path.nodes_.back().type_ = exploration_path_ns::NodeType::GLOBAL_VIEWPOINT;
            ordered_cell_indices.clear();
            ordered_cell_indices.push_back(last_goal_cell_id);
            std::cout << "当前位置" << std::endl;
            std::cout << "全局路径机器人位置: (" << global_path_robot_position.x << ", " << global_path_robot_position.y << ", " << global_path_robot_position.z << ")" << std::endl;
            robot_statu_ = RobotStatus::Request;
            Request_breakpoint_count_ = 0;
            std::cout << "请求状态正常执行" << std::endl;
            std::cout << "目标为探索状态" << std::endl;
            last_global_goal_path_ = global_path;
            return global_path;
          }
          std::cout << "路径小于2   A*  求解失败" << std::endl;
          is_reachable = false;
        }

        if (!is_reachable)  
        {
          std::cout << "当前点不可通行  断点了" << std::endl;
          if (Request_breakpoint_count_ == 10) 
          {
            subspaces_world_->GetCell(last_goal_cell_id).SetStatus(CellStatus::COVERED);
            std::cout << std::endl
                      << "\033[1;32m"
                      << "目标是探索状态，但目标点是误点，置为已探索"
                      << "\033[0m" << std::endl;
          }

          if (!viewpoint_manager->InLocalPlanningHorizon(last_path.nodes_.back().position_))
          {
            Request_breakpoint_count_++;
            double dist_from_robot = DBL_MAX;
            int min_index = -1;
            for (int node_ind = 0; node_ind < last_path.nodes_.size(); node_ind++)
            {
              geometry_msgs::msg::Point min_point;
              min_point.x = last_path.nodes_[node_ind].position_[0];
              min_point.y = last_path.nodes_[node_ind].position_[1];
              min_point.z = last_path.nodes_[node_ind].position_[2];
              double dist =
                  misc_utils_ns::PointXYZDist<geometry_msgs::msg::Point, geometry_msgs::msg::Point>(robot_position_, min_point);
              if (dist < dist_from_robot)
              {
                dist_from_robot = dist;
                min_index = node_ind;
              }
            }
            ...
            if (min_index != -1)
            {
              exploration_path_ns::Node start_node;
              start_node.position_[0] = robot_position_.x;
              start_node.position_[1] = robot_position_.y;
              start_node.position_[2] = robot_position_.z;
              start_node.global_subspace_index_ = cur_robot_cell_ind_;
              start_node.type_ = exploration_path_ns::NodeType::ROBOT;
              global_path.Append(start_node);
              for (int i = min_index; i < last_path.nodes_.size(); i++)
              {
                global_path.Append(last_path.nodes_[i]);
              }
              ordered_cell_indices.clear();
              ordered_cell_indices.push_back(last_goal_cell_id);
              // last_global_goal_path_=global_path;
              robot_statu_ = RobotStatus::Request;
              std::cout << "目标点局部 规划 之外    使用之前的路径" << std::endl;
              last_global_goal_path_ = global_path;
              return global_path;
            }
          }
    ...
    std::cout << std::endl
              ...
              << "切换状态，重置探索逻辑"
              << "\033[0m" << std::endl;
    if (is_global_tsp == false)
    {
      std::cout << std::endl
                << "\033[1;32m"
                << "切换状态，从请求状态  变为探索状态，执行局部规划"
                << "\033[0m" << std::endl;
      return global_path;
    }
    return global_path;
  }
  if (is_fuse_ && robot_statu_ == RobotStatus::Far_planner) 
  {
    exploration_path_ns::ExplorationPath last_path = last_global_goal_path_;
    if (last_path.GetLength() > 0 && exploring_cell_indices_world.size() == 1)  
    {
  ...
  if ((exploring_cell_indices_world.size() == 1 && num_world_neighbor_cell_reachable == 0) || (is_global_tsp && exploring_cell_indices_world.size() == 1)) 
  {
    is_fuse_ = GetGlobalMtspGrapher_FusedGrapher_pro(
        exploring_cell_indices, exploring_cell_positions, distance_matrix, exploring_cell_indices_MTSP,
        exploring_cell_positions_MTSP, distance_matrix_MTSP, robot_position_id_on_exploring_cell_positions_MTSP,
        MTSP_subgrapher_map_, keypose_graph, Fuse_Grapher_, positions_fuse_grapher_node_id);
  }
  std::cout << " is_fuse_ =   " << is_fuse_ << std::endl;

  return_home_ = false;
  if (robot_statu_ == RobotStatus::Exploring && exploring_cell_indices_world.size() == 1 && !is_fuse_ &&
      num_world_neighbor_cell_reachable == 0) 
  {
    // return_home_ = true;
  ...
  {
    tsp_solver_ns::DataModel data_model;                      
    data_model.distance_matrix = distance_matrix_world;        
    data_model.depot = exploring_cell_positions_world.size() - 1; 
  ...
  { 
    int robot_num = robot_position_id_on_exploring_cell_positions_MTSP.size();
    tsp_solver_ns::DataModel data_model;            
    data_model.distance_matrix = distance_matrix_MTSP; 
    data_model.num_vehicles = robot_num;
    int num = robot_position_id_on_exploring_cell_positions_MTSP.size();
    for (int i = 0; i < num; i++)
    {
      int id = robot_position_id_on_exploring_cell_positions_MTSP[i];
      std::pair<int, int> id_(id, id);  
      data_model.depot_M.emplace_back(id_);  
    }
    data_model.is_MTSP = true; 
    tsp_solver_ns::TSPSolver tsp_solver(data_model);
    tsp_solver.Solve();
    tsp_solver.getSolutionNodeIndex(node_index, false);  //
    ordered_cell_indices.clear();
  ...
  if (is_fuse_)  
  {
    ...
    {
      std::cout << "   做mtsp   " << std::endl;
      int robot_num = robot_position_id_on_exploring_cell_positions_MTSP.size();  
      int exploring_num =
          distance_matrix_MTSP.size() - robot_position_id_on_exploring_cell_positions_MTSP.size();  
      exploration_path_ns::Node node_goal;
      int goal_ind;
      if (node_index.size() <= 2 && exploring_num > 0)  
      {
        std::cout << "   mtsp   没有分配  探索补偿目标点   " << std::endl;
        std::vector<int> distance_vector = distance_matrix_MTSP[robot_position_id_on_exploring_cell_positions_MTSP[0]];
        std::vector<geometry_msgs::msg::Point> goal_position;
        std::vector<int> goal_position_distance;
      ...
      {
        std::cout << "   mtsp   正常分配目标点     " << std::endl;
        int goal_ind_ = node_index[1];
      ...
      node_local.global_subspace_index_ =
          GetCellInd(exploring_cell_positions[robot_ind].x, exploring_cell_positions[robot_ind].y,
                     exploring_cell_positions[robot_ind].z);
      global_path.Append(node_local);
      int goal_fuse_grapher_node_id = positions_fuse_grapher_node_id[goal_ind];
      int robot_grapher_node_id = 0; 
      robot_grapher_node_id = positions_fuse_grapher_node_id[robot_position_id_on_exploring_cell_positions_MTSP[0]];
      if (positions_fuse_grapher_node_id[robot_position_id_on_exploring_cell_positions_MTSP[0]] == 0)
      {
        std::cout << "\033[1;32m"
                  << "  融图机器人排序正确"
                  << "\033[0m" << std::endl;
      ...
      std::vector<int> path =
          GetShortestPath_Fuse_Grapher_pro(Fuse_Grapher_, robot_grapher_node_id, goal_fuse_grapher_node_id);
      std::cout << "起点  robot_grapher_node_id :   " << robot_grapher_node_id << std::endl;
      std::cout << "目标点  goal_fuse_grapher_node_id :  " << goal_fuse_grapher_node_id << std::endl;
      std::cout << "路径个数   :   " << path.size() << "     数量为3 才是正确的" << std::endl;
      for (int k = 0; k < path.size(); k++)
      {
        std::cout << "     " << path[k] << std::endl;
      }
      geometry_msgs::msg::Point Intermediate_node;  //中间节点坐标
      int Intermediate_node_other_robot_id = 0;
      geometry_msgs::msg::Point goal_node;  //目标节点
      goal_node = exploring_cell_positions[goal_ind];
      // std::cout << "goal_node    " << goal_node << std::endl;

      if (path.size() >= 3)  //
      {
        //  path[i]    从后往前查看，第一个点  不在  规划里面     positions_fuse_grapher_node_id   里面
        // debug
        for (int i = 0; i < positions_fuse_grapher_node_id.size(); i++)
        {
          std::cout << positions_fuse_grapher_node_id[i] << "     ";
        }
        std::cout << std::endl;
        ...
        for (int i = path.size() - 1; i >= 0; i--)
        {
          ...
          //  中间点可以是已探索的 其他机器人位置
          if ((Fuse_Grapher_.fuse_nodes_[path[i]].is_Explored &&
               Fuse_Grapher_.fuse_nodes_[path[i]].robot_id_ == Fuse_Grapher_.fuse_nodes_[path.back()].robot_id_) ||
              path[i] == 1)  //  中间点是对于机器人的已探索点  或是起点 序号为1
          {
            //当  path[i] 路径 不在   选用于MTSP的  集和上    positions_fuse_grapher_node_id   说明点是
            //外部已探索，用于中间点。
            Intermediate_node = Fuse_Grapher_.fuse_nodes_[path[i]].position_;
            Intermediate_node_other_robot_id = Fuse_Grapher_.fuse_nodes_[path.back()].robot_id_;  //  目标机器人ID
            Request_robot_id_ = Fuse_Grapher_.fuse_nodes_[path.back()].robot_id_;  //请求路经的机器人ID
            std::cout << "成功找到外部已探索点  做中间结点    " << path[i] << std::endl;
            // std::cout << "中间结点： " << Intermediate_node << std::endl;
            break;
          }
        }
      }
      ...
      {
        std::cout << "路径个数   小于 3   有问题   本地不会直连外部未探索点" << std::endl;
      }
      bool is_Intermediate_node_succeeded = false;
      if (Intermediate_node_other_robot_id != 0)
      {
        std::cout << "寻找中间结点  成功" << std::endl;
        is_Intermediate_node_succeeded = true;
        std::cout << "Intermediate_node_other_robot_id  " << Intermediate_node_other_robot_id << std::endl;
      }
      /* 新修改 */
      auto request_path = std::make_shared<tare_planner::srv::RequestPath::Request>();  
      auto response_path = std::make_shared<tare_planner::srv::RequestPath::Response>(); 

      request_path->start = Intermediate_node;
      request_path->goal = exploring_cell_positions[goal_ind];
      std::cout << "请求路径" << std::endl;
      bool is_request_succeeded = false;
      if (is_Intermediate_node_succeeded)
      {
        std::cout << "请求路径" << std::endl;
        // is_request_succeeded = request_path_client_list_[Intermediate_node_other_robot_id - 1]->async_send_request(request_path);
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        auto future = request_path_client_list_[Intermediate_node_other_robot_id - 1]->async_send_request(request_path);
        if(future.wait_for(std::chrono::seconds(1)) == std::future_status::ready)
        {
          auto response_test = future.get();
          if(response_test)
          {
        ...
        {
          std::cout << "请求路径失败   切换起点做请求" << std::endl;
          geometry_msgs::msg::Point home_position_ = keypose_graph->GetFirstKeyposePosition();  //位置数据
          request_path->start = home_position_;
          response_path->path_return.poses.clear();
          // is_request_succeeded = request_path_client_list_[Intermediate_node_other_robot_id - 1]->async_send_request(request_path);
          //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
          auto future = request_path_client_list_[Intermediate_node_other_robot_id - 1]->async_send_request(request_path);
          if(future.wait_for(std::chrono::seconds(1)) == std::future_status::ready)
      ...
      if (is_request_succeeded && response_path->path_return.poses.size() >= 2)
      {
        std::cout << "请求路径   成功" << std::endl;
        nav_msgs::msg::Path this_path = response_path->path_return;
        std::cout << "返回路径长度 ：   " << this_path.poses.size() << std::endl;
        exploration_path_ns::ExplorationPath Intermediate_goal_path; 
        bool is_connect = UpDateIntermediate2GoalPath(this_path, keypose_graph);
        std::cout << "请求路径   成功" << std::endl;
        exploration_path_ns::ExplorationPath robotpose_Intermediate_path;
        nav_msgs::msg::Path keypose_path; 
        if (is_connect)        
        {
          std::cout << "连接成功" << std::endl;
          keypose_graph->GetShortestPath(cur_position, goal_node, true, keypose_path, false);  

          std::cout << "求 cur_position-> goal_node  路径成功" << std::endl;
          std::cout << "keypose_path.size()    " << keypose_path.poses.size() << std::endl;
          // std::cout << keypose_path << std::endl;
          if (keypose_path.poses.empty())
          {
            is_connect = false;
          }
        }
        if (!is_connect)
        {  //没有连接成功   就分开求两段      当前机器人->  this_path[0]   拼接  this_path[0]->this_path.back()
          std::cout << "连接失败" << std::endl;
          keypose_graph->GetShortestPath(cur_position, Intermediate_node, true, keypose_path, false);
          std::cout << "求 cur_position-> Intermediate_node  路径成功" << std::endl;
          //  keypose_path  + this_path
          for (int i = 0; i < this_path.poses.size(); i++)
          {
            keypose_path.poses.push_back(this_path.poses[i]);
          }
        }
        //将  keypose_path  变成      global_path
        std::cout << "加入 路径keypose_path" << std::endl;
        if (keypose_path.poses.size() > 2)
        {
          for (int i = 0; i < keypose_path.poses.size() - 1; i++)
          {
            geometry_msgs::msg::Point temp;
            temp = keypose_path.poses[i].pose.position;
            exploration_path_ns::Node node_temp(temp, exploration_path_ns::NodeType::GLOBAL_VIA_POINT);  //当前位置
            node_temp.type_ = exploration_path_ns::NodeType::GLOBAL_VIA_POINT;
            node_temp.global_subspace_index_ = GetCellInd(temp.x, temp.y, temp.z);
            global_path.Append(node_temp);
            ordered_cell_indices.push_back(node_temp.global_subspace_index_);
          }
          std::cout << "加入 路径keypose_path成功" << std::endl;
          //加入前置  得到的目标点
          global_path.Append(node_goal);
          ordered_cell_indices.push_back(node_goal.global_subspace_index_);
          //更新机器人状态
          std::cout << "请求状态  请求路径成功" << std::endl;
          last_global_goal_path_ = global_path;
          Request_breakpoint_count_ = 0;
          robot_statu_ = RobotStatus::Request;
          return global_path;
      ...
      //运行到这   就说明获取失败
      std::cout << "请求路径   失败  采取far方式" << std::endl;
      //加入目标
      global_path.Append(node_goal);
      //补偿加入
      global_path.Append(global_path.nodes_[0]);
      ...
      //打印     到达目标逻辑 的最终结果
      std::cout << "\033[1;32m"
                << "  做MTSP  far   目标点  Cell_id ：    " << node_goal.global_subspace_index_ << "\033[0m"
                << std::endl;
      last_global_goal_path_ = global_path;
      //更新机器人状态
...
exploration_path_ns::ExplorationPath
GridWorld::SolveGlobalTSP_max(const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>& viewpoint_manager,
                              std::vector<int>& ordered_cell_indices, std::vector<double>& M_TSP_subgrapher_code,
                              std::map<int, std::vector<double>>& MTSP_subgrapher_map_, bool& is_global_tsp,
                              ...
                              std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph) 
{
  ...
  //  完善     持续  做       request      far_planner    逻辑。
  if (is_fuse_ && robot_statu_ == RobotStatus::Request)  //上一次  是  请求路径 并得到路径 加载到  keyposegrapher 里面
  {
    exploration_path_ns::ExplorationPath last_path = last_global_goal_path_;
    geometry_msgs::msg::Point goal_point;
    goal_point.x = last_path.nodes_.back().position_[0];
    ...
    if (last_path.GetLength() > 0)
    {
      ...
      // 一、  网格是否探索状态
      if (subspaces_world_->GetCell(last_goal_cell_id).GetStatus() ==
          CellStatus::EXPLORING)  //目标点的全局还是探索状态，
      {
        std::cout << "目标点   还是探索状态" << std::endl;
        double distance_robot_goal = last_path.GetLength();
        ...
        //求当前点最近的  keyposegrapher  id    判断是不是连通的
        if (is_reachable)  //可通行
        {
          //计算当前机器人位置 和  目标点位置 的路径  返回即可
          nav_msgs::msg::Path this_path;
          keypose_graph->GetShortestPath(global_path_robot_position, goal_point, true, this_path, false);
          if (this_path.poses.size() >= 2)
          {
        ...
        if (!is_reachable)  //  目标点  是探索    但不可通行  说明是误点 更新网格状态 变成已探索
        {
          std::cout << "当前点不可通行  断点了" << std::endl;
          //  若    目标点      在局部规划范围之外     肯定不会是误点，  一定是连接中断了
          if (Request_breakpoint_count_ == 10)  //断点累计10次    更加精确的是  在同一个  cell  断点，不同cell
                                                //断点无所谓
          {
            //  当前点是误点
            subspaces_world_->GetCell(last_goal_cell_id).SetStatus(CellStatus::COVERED);
          ...
          if (!viewpoint_manager->InLocalPlanningHorizon(last_path.nodes_.back().position_))
          {
            int viewpoint_ind = viewpoint_manager->GetViewPointInd(last_path.nodes_.back().position_);
            if (viewpoint_manager->InRange(viewpoint_ind))  //在局部范围框内（有邻接cell 组成），且不在
                                                            //局部规划范围内（局部规划范围  是候选视点组成的）
            {
              Request_breakpoint_count_++;
            }
            //连接中断  ，计算  离   当前点   最近的点 路径点 做起点  传出。
            double dist_from_robot = DBL_MAX;
            int min_index = -1;
            for (int node_ind = 0; node_ind < last_path.nodes_.size(); node_ind++)
            {
  ...
  if (is_fuse_ && robot_statu_ == RobotStatus::Far_planner)
  {
    ...
    if (last_path.GetLength() > 0)
    {
      ...
      if (subspaces_world_->GetCell(last_goal_cell_id).GetStatus() ==
          CellStatus::EXPLORING) 
      {
        if (Request_robot_id_ != 0)
        {
          geometry_msgs::msg::Point goal_node;
          geometry_msgs::msg::Point cur_position;
          geometry_msgs::msg::Point Intermediate_node;
          Intermediate_node = Intermediate_node_;
          cur_position = global_path_robot_position;

          /* 新修改 */
          auto request_path = std::make_shared<tare_planner::srv::RequestPath::Request>();  
          auto response_path = std::make_shared<tare_planner::srv::RequestPath::Response>(); 
          request_path->start = Intermediate_node;  // 直接访问start字段（无.Request）
          request_path->goal.x = last_path.nodes_.back().position_[0];
          request_path->goal.y = last_path.nodes_.back().position_[1];
          request_path->goal.z = last_path.nodes_.back().position_[2];
          goal_node = request_path->goal;
          std::cout << "far 请求路径" << std::endl;
          bool is_request_succeeded = false;
          // is_request_succeeded = request_path_client_list_[Request_robot_id_ - 1]->async_send_request(request_path);
          //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
          auto future = request_path_client_list_[Request_robot_id_ - 1]->async_send_request(request_path);
          if(future.wait_for(std::chrono::seconds(1)) == std::future_status::ready)
          {
            auto response_test = future.get();
            if(response_test)
          ...
          if (!is_request_succeeded)
          {
            std::cout << "far 请求路径失败   切换起点做请求" << std::endl;
            geometry_msgs::msg::Point home_position_ = keypose_graph->GetFirstKeyposePosition();  //位置数据
            request_path->start = home_position_;
            response_path->path_return.poses.clear();
            /* 新修改 */
          ...
          if (is_request_succeeded && response_path->path_return.poses.size() >= 2)
          {
            std::cout << "far 请求路径   成功" << std::endl;
            nav_msgs::msg::Path this_path = response_path->path_return;
            std::cout << "far 返回路径长度:   " << this_path.poses.size() << std::endl;
            exploration_path_ns::ExplorationPath Intermediate_goal_path;  
            bool is_connect = UpDateIntermediate2GoalPath(this_path, keypose_graph);
            std::cout << "far请求路径   成功" << std::endl;
            exploration_path_ns::ExplorationPath robotpose_Intermediate_path;
            nav_msgs::msg::Path keypose_path;  //路径
            if (is_connect)             
            {
            ...
            if (keypose_path.poses.size() > 2)
            {
              ...
              std::cout << "加入 路径keypose_path成功" << std::endl;
              exploration_path_ns::Node node_goal(goal_node,
                                                  exploration_path_ns::NodeType::GLOBAL_VIEWPOINT);  //当前位置
              node_goal.global_subspace_index_ = GetCellInd(goal_node.x, goal_node.y, goal_node.z);
              global_path.Append(node_goal);
              ordered_cell_indices.push_back(node_goal.global_subspace_index_);
              //更新机器人状态
              std::cout << "请求状态  请求路径成功" << std::endl;
  ...
  //由于    融合图 数值不稳定     在达到目标地逻辑中  倾向于  执行成功一次之后在融合判断     防止  反复振荡。
  if (robot_statu_ == RobotStatus::Global_tsp)
  {
    // global_tsp  有两种结果：
    // 1.  是融合失败本地做  tsp  路经很长
    // 2.  是融合成功选择本地的一个点     路经较短
    //都直接选用  路经上第一个  属性为   exploration_path_ns::NodeType::GLOBAL_VIEWPOINT   判定是否为已探索
    exploration_path_ns::ExplorationPath last_path = last_global_goal_path_;
    //从前查询目标点：
    geometry_msgs::msg::Point pre_goal;
    // 从后查询目标点：
    geometry_msgs::msg::Point back_goal;
    for (int i = 0; i < last_path.nodes_.size(); i++)
    {
      if (last_path.nodes_[i].type_ == exploration_path_ns::NodeType::GLOBAL_VIEWPOINT)
      {
        pre_goal.x = last_path.nodes_[i].position_[0];
        pre_goal.y = last_path.nodes_[i].position_[1];
        pre_goal.z = last_path.nodes_[i].position_[2];
      }
    }
    for (int i = last_path.nodes_.size() - 1; i >= 0; i--)
    {
      if (last_path.nodes_[i].type_ == exploration_path_ns::NodeType::GLOBAL_VIEWPOINT)
      {
        back_goal.x = last_path.nodes_[i].position_[0];
        back_goal.y = last_path.nodes_[i].position_[1];
        back_goal.z = last_path.nodes_[i].position_[2];
      }
    }
    //查询前后点cell statu  一旦某一个是非  探索状态  则重置为  探索状态
    int pre_goal_cell_index = GetCellInd(pre_goal.x, pre_goal.y, pre_goal.z);
    int back_goal_cell_index = GetCellInd(back_goal.x, back_goal.y, back_goal.z);
    std::cout << "前后目标： " << pre_goal_cell_index << "    " << back_goal_cell_index << std::endl;
    //  若前后两个目标都是探索状态， 则说明还没有到达目标点  得计算路经
    if (subspaces_world_->GetCell(pre_goal_cell_index).GetStatus() == CellStatus::EXPLORING &&
        subspaces_world_->GetCell(back_goal_cell_index).GetStatus() == CellStatus::EXPLORING)
    {
      //当前到pre_goal;
      nav_msgs::msg::Path robot2pregoal_path;
      keypose_graph->GetShortestPath(global_path_robot_position, pre_goal, true, robot2pregoal_path, false);
      std::cout << "  robot2pregoal_path.size()   " << robot2pregoal_path.poses.size() << std::endl;
      nav_msgs::msg::Path pregoal2backgoal_path;
      bool flag_pre = false;
      bool flag_pre_back = false;
      bool flag_back = false;
      if (pre_goal_cell_index != back_goal_cell_index)
      {
        keypose_graph->GetShortestPath(pre_goal, back_goal, true, pregoal2backgoal_path, false);
        std::cout << "  pregoal2backgoal_path.size()   " << pregoal2backgoal_path.poses.size() << std::endl;
      }
      else
      {
        pregoal2backgoal_path.poses.clear();
      }
      nav_msgs::msg::Path backgoal2robot_path;
      keypose_graph->GetShortestPath(back_goal, global_path_robot_position, true, backgoal2robot_path, false);
      std::cout << "  backgoal2robot_path.size()   " << backgoal2robot_path.poses.size() << std::endl;
      nav_msgs::msg::Path global_path_;
      //加三段
      if (robot2pregoal_path.poses.size() >= 2)
      {
        flag_pre = true;
      }
      std::cout << " 加入 robot2pregoal_path   " << robot2pregoal_path.poses.size() << std::endl;
      if (backgoal2robot_path.poses.size() >= 2)
      {
        flag_back = true;
      }
      std::cout << " 加入backgoal2robot_path   " << backgoal2robot_path.poses.size() << std::endl;
      if (pregoal2backgoal_path.poses.size() >= 2 && flag_back && flag_pre)
      {
        flag_pre_back = true;
      }
      ...
      if (flag_pre)
      {
        for (int i = 0; i < robot2pregoal_path.poses.size(); i++)
        {
          global_path_.poses.push_back(robot2pregoal_path.poses[i]);
        }
      }
      if (flag_pre_back)
      {
        for (int i = 1; i < pregoal2backgoal_path.poses.size() - 1; i++)
        {
          global_path_.poses.push_back(pregoal2backgoal_path.poses[i]);
        }
        std::cout << " 加入pregoal2backgoal_path   " << pregoal2backgoal_path.poses.size() << std::endl;
      }
      if (flag_back)
      {
        for (int i = 0; i < backgoal2robot_path.poses.size(); i++)
        {
          global_path_.poses.push_back(backgoal2robot_path.poses[i]);
        }
      }

      if (global_path_.poses.size() >= 2)
      {
        global_path.FromPath(global_path_);
        global_path.nodes_.front().type_ = exploration_path_ns::NodeType::ROBOT;

        if (flag_pre && flag_back)  //返回三种结果
        {
          for (int i = 1; i < global_path.nodes_.size() - 1; i++)  //全置为  全局路经经过点
          {
            global_path.nodes_[i].type_ = exploration_path_ns::NodeType::GLOBAL_VIA_POINT;
          }
          //修正其中两个  全局点类型
          global_path.nodes_[robot2pregoal_path.poses.size() - 1].type_ =
              exploration_path_ns::NodeType::GLOBAL_VIEWPOINT;
          global_path.nodes_[(global_path.nodes_.size() - 1) - (backgoal2robot_path.poses.size() - 1)].type_ =
              exploration_path_ns::NodeType::GLOBAL_VIEWPOINT;
          global_path.nodes_.back().type_ = exploration_path_ns::NodeType::ROBOT;
        }
        else if ((flag_pre && !flag_pre_back && !flag_back))
        {
          for (int i = 1; i < global_path.nodes_.size() - 1; i++)  //全置为  全局路经经过点
          {
            global_path.nodes_[i].type_ = exploration_path_ns::NodeType::GLOBAL_VIA_POINT;
        ...
        {
          std::reverse(global_path.nodes_.begin(), global_path.nodes_.end());
          for (int i = 1; i < global_path.nodes_.size() - 1; i++)  //全置为  全局路经经过点
          {
            global_path.nodes_[i].type_ = exploration_path_ns::NodeType::GLOBAL_VIA_POINT;
          }
        ...
        Request_breakpoint_count_ = 0;
        std::cout << "  执行Global_tsp" << std::endl;
        return global_path;
      }
      std::cout << "  重复  Global_tsp  失败" << std::endl;
    }
    // 若有已经是目标点的  就重置  重新做融合再判断
    if (is_global_tsp == false)
    {
      Request_breakpoint_count_ = 0;
      robot_statu_ = RobotStatus::Exploring;
  ...
  //*****************************融图***********************************//
  // 修改   使用  distance_matrix_world  及  exploring_cell_positions_world  代替本地
  is_fuse_ = GetGlobalMtspGrapher_FusedGrapher_max(
      exploring_cell_indices_world, exploring_cell_positions_world, distance_matrix_world, exploring_cell_indices_MTSP,
      exploring_cell_positions_MTSP, distance_matrix_MTSP, robot_position_id_on_exploring_cell_positions_MTSP,
      MTSP_subgrapher_map_, keypose_graph, Fuse_Grapher_, positions_fuse_grapher_node_id);
  ...
  //  返回起点
  return_home_ = false;
  if (exploring_cell_indices_world.size() == 1 && !is_fuse_ &&
      num_world_neighbor_cell_reachable == 0)  //融合失败 且 没有可到达的点  就算探索完毕
  {
    // return_home_ = true;
    std::cout << "return_home_    " << return_home_ << std::endl;
  ...
  {  //成功做 MTSP
    ...
    data_model.num_vehicles = robot_num;
    int num = robot_position_id_on_exploring_cell_positions_MTSP.size();
    //   robot_position_id_on_exploring_cell_positions_MTSP[0]  是机器人  在  exploring_cell_positions_MTSP 上的索引
    for (int i = 0; i < num; i++)
    {
      int id = robot_position_id_on_exploring_cell_positions_MTSP[i];
  ...
  //  融合图传出的数据 必须更多，得知道  每个外部点       对应的机器人编号   方便使用  服务寻求返回路径
  //  且得知道  对应外部点经过哪个  本地节点     本地节点和目标点的路径
  //  本地位置-> 本地节点->外部目标点
  ...
  {
    if (!use_keypose_graph_ || keypose_graph == nullptr ||
        keypose_graph->GetNodeNum() == 0)  //如果  不使用关键位姿图或者是初始化的时候
    {
      for (int i = 0; i < node_index.size(); i++)  //对于每个排序的点
      {
        int cell_ind = node_index[i];
    ...
    else  //如果使用关键位姿图
    {
      ...
      int robot_num = robot_position_id_on_exploring_cell_positions_MTSP.size();  //机器人数量
      int exploring_num =
          distance_matrix_MTSP.size() - robot_position_id_on_exploring_cell_positions_MTSP.size();  //外部未探索点数量
      exploration_path_ns::Node node_goal;
      int goal_ind;
      if (node_index.size() <= 2 && exploring_num > 0)  // MTSP       没有给本地机器人分配目标点
      ...
      else
      {
        ...
        if (distance_matrix_MTSP[robot_position_id_on_exploring_cell_positions_MTSP[0]][goal_ind_] >
            distance_matrix_MTSP[robot_position_id_on_exploring_cell_positions_MTSP[0]][goal_ind_compare])
        {
      ...
      //  判定目标属于  哪个机器人  若是本地机器人   就直接使用  Global_tsp
      if (Fuse_Grapher_.fuse_nodes_[positions_fuse_grapher_node_id[goal_ind]].robot_id_ == 0)  //目标点是本地
      {
        std::cout << "   mtsp   选择本地发现的全局探索点     " << std::endl;
        geometry_msgs::msg::Point goal_position;  //目标节点
        goal_position = exploring_cell_positions[goal_ind];
        nav_msgs::msg::Path goal_path;
        if (!use_keypose_graph_ || keypose_graph == nullptr || keypose_graph->GetNodeNum() == 0)
        {
          geometry_msgs::msg::PoseStamped robot_pose;
          robot_pose.pose.position = robot_position_;
          ...
          goal_path.poses.push_back(goal_pose);
        }
        else
        {
          keypose_graph->GetShortestPath(global_path_robot_position, goal_position, true, goal_path, false);
          if (goal_path.poses.size() >= 2)
          {
            global_path.FromPath(goal_path);
            global_path.nodes_.front().type_ = exploration_path_ns::NodeType::ROBOT;
      ...
      if (path.size() >= 3)  //
      {
        ...
        for (int i = path.size() - 1; i >= 0; i--)
        {
              ...
              path[i] == 1)  //  中间点是对于机器人的已探索点  或是起点 序号为1
          {
            ...
            //外部已探索，用于中间点。
            Intermediate_node = Fuse_Grapher_.fuse_nodes_[path[i]].position_;
            Intermediate_node_ = Fuse_Grapher_.fuse_nodes_[path[i]].position_;
            Intermediate_node_other_robot_id = Fuse_Grapher_.fuse_nodes_[path.back()].robot_id_;  //  目标机器人ID
            Request_robot_id_ = Fuse_Grapher_.fuse_nodes_[path.back()].robot_id_;
            std::cout << "成功找到外部已探索点  做中间结点    " << path[i] << std::endl;
            // std::cout << "中间结点： " << Intermediate_node << std::endl;
            break;
          }
      ...
      if (is_Intermediate_node_succeeded)
      {
        ...
        if (!is_request_succeeded)
        {
          ...
          request_path->start = home_position_;
          response_path->path_return.poses.clear();
          // is_request_succeeded = request_path_client_list_[Request_robot_id_ - 1];
          //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
          auto future = request_path_client_list_[Intermediate_node_other_robot_id - 1]->async_send_request(request_path);
          if(future.wait_for(std::chrono::seconds(1)) == std::future_status::ready)
          {
      ...
      if (is_request_succeeded && response_path->path_return.poses.size() >= 2)  //请求成功，  请求的时候会阻塞，但一般 运行会很快。
      {
        std::cout << "请求路径   成功" << std::endl;
        nav_msgs::msg::Path this_path = response_path->path_return;
        exploration_path_ns::ExplorationPath Intermediate_goal_path;  //中间点到目标点
        bool is_connect = UpDateIntermediate2GoalPath(this_path, keypose_graph);
        std::cout << "请求路径   成功" << std::endl;
        exploration_path_ns::ExplorationPath robotpose_Intermediate_path;
        nav_msgs::msg::Path keypose_path;  //路径
...
！！！！！！！！！！！！！！！！！！！！分界线！！！！！！！！！！！！！！！！！！
                                                                    使用世界网格做目标分配（MTSP）
                                                                  SolveGlobalTSP_grid_world_graph

***************************************************************************************/
exploration_path_ns::ExplorationPath GridWorld::SolveGlobalTSP_grid_graph(
    const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>& viewpoint_manager,
    std::vector<int>& ordered_cell_indices, std::vector<double>& M_TSP_subgrapher_code,
    std::map<int, std::vector<double>>& MTSP_subgrapher_map_, bool& is_global_tsp,
    std::vector<rclcpp::Client<tare_planner::srv::RequestPath>::SharedPtr>& request_path_client_list_, // 替换为ROS2
    std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)  //添加编码  传到外部去发送
{
  ...
  for (int i = 0; i < subspaces_->GetCellNumber(); i++) 
  {
    if (subspaces_world_->GetCell(i).GetStatus() == CellStatus::EXPLORING) 
    {
      if (std::find(neighbor_cell_indices_.begin(), neighbor_cell_indices_.end(), i) == neighbor_cell_indices_.end() ||
          (subspaces_->GetCell(i).GetViewPointIndices().empty() &&
           subspaces_->GetCell(i).GetVisitCount() > 1))  //  i  不在邻接单元里面
  ...
  // 添加      本地邻接范围内是否有可到达的探索点，个数  用于判定是是否返回
  num_local_neighbor_cell_reachable = 0;
  for (int i = 0; i < neighbor_cell_indices_.size(); i++)
  {
    int cell_id = neighbor_cell_indices_[i];
  ...
  /*                           编码布
        将  distance_matrix   和     exploring_cell_positions    编码    共享出去
        std::vector<std::vector<int>>   distance_matrix
       std::vector<geometry_msgs::Point>  exploring_cell_positions      编码共享发送
  */
  M_TSP_subgrapher_code =
      GetMtspSubgrapherCode(exploring_cell_positions_world, distance_matrix_world);  //返回个数  1+3*n+n*n
  ...
  std::vector<int> robot_position_id_on_exploring_cell_positions_MTSP;  // 机器人位置  在 exploring_cell_positions_MTSP
                                                                        // 上的ID 索引
  std::vector<int> positions_fuse_grapher_node_id;  // MTSP  传出点在融合图上  的序号  方便查找。
  ...
  //采用   使用  far_planner
  if (robot_statu_ ==
      RobotStatus::Far_planner)
  {
    std::cout << "far_breakpoint_coint_  " << far_breakpoint_coint_ << std::endl;
    exploration_path_ns::ExplorationPath last_path = last_global_goal_path_;
    if (last_path.GetLength() > 0)
    {
      //误点判定
      int last_goal_cell_id = GetCellInd(goal_position_.x, goal_position_.y, goal_position_.z);
      if (far_breakpoint_coint_ >= 2)  //判定为误点
      {
        robot_statu_ = RobotStatus::Exploring;
        subspaces_world_->GetCell(last_goal_cell_id).SetStatus(CellStatus::COVERED);
      ...
      if (subspaces_world_->GetCell(last_goal_cell_id).GetStatus() ==
          CellStatus::EXPLORING) 
      {
        geometry_msgs::msg::Point goal_position_graph;
        if (have_Navigation_node_)
        {
          goal_position_graph = last_goal_node_.position_;
        }
        else
        {
          goal_position_graph = goal_position_;
        }

        double distance_robot_last_navi = misc_utils_ns::PointXYZDist<geometry_msgs::msg::Point, geometry_msgs::msg::Point>(
            goal_position_graph, robot_position_);
        Eigen::Vector3d temp_check_position =
            Eigen::Vector3d(goal_position_graph.x, goal_position_graph.y, goal_position_graph.z);
        int viewpoint_ind = viewpoint_manager->GetViewPointInd(temp_check_position);  //得到  点 的  局部规划id

        //切换导航点
        std::cout << "  far   切换导航点  " << std::endl;
        exploration_path_ns::Node node_local(robot_position_, exploration_path_ns::NodeType::ROBOT);  //当前位置
        node_local.global_subspace_index_ = cur_robot_cell_ind_;
        global_path.Append(node_local);
        temp_check_position = Eigen::Vector3d(goal_position_.x, goal_position_.y, goal_position_.z);
        viewpoint_ind = viewpoint_manager->GetViewPointInd(temp_check_position);  //得到  点 的  局部规划id
        if (!viewpoint_manager->InRange(viewpoint_ind))  //目标点不在局部  范围内   选用导航点
        {
          std::vector<int> path_node_index;
          int from_index = MTSP_grid_graph_.last_robot_node_index_[cur_robot_id_];
          int temp_node_index = -1;
          double temp_node_dist = DBL_MAX;
          std::cout << "MTSP_graph  得到距离  外部探索点  最近的结点" << std::endl;
          MTSP_grid_graph_.GetClosestConnectedNodeIndAndDistance(goal_position_, temp_node_index, temp_node_dist); 
          int to_index = temp_node_index;
          path_node_index = GetShortestPath_MTSP_grid_graph_path(from_index, to_index);
          int Navigation_node_index;
          //得到导航点
          bool use_Navigation_node = true;
          Navigation_node_index = GetNavigationIndexFromMtspGridGraph(path_node_index, viewpoint_manager, keypose_graph, use_Navigation_node);
          if (Navigation_node_index != -1 && path_node_index.size() >= 2 && use_Navigation_node)
          {
            exploration_path_ns::Node Navigation_node(
                MTSP_grid_graph_.MTSP_graph_nodes_[Navigation_node_index].position_,
                exploration_path_ns::NodeType::GLOBAL_VIEWPOINT);
            Navigation_node.global_subspace_index_ = MTSP_grid_graph_.MTSP_graph_nodes_[Navigation_node_index].cell_id_;
            global_path.Append(Navigation_node);
            global_path.Append(global_path.nodes_[0]);
            ordered_cell_indices.push_back(last_goal_cell_id);
            //打印     到达目标逻辑 的最终结果
            std::cout << "\033[1;32m"
                      << "  做MTSP  far   目标点  Cell_id ：    " << Navigation_node.global_subspace_index_ << "\033[0m"
                      << std::endl;
            last_global_goal_path_ = global_path;
            last_far_navigation_node_ = MTSP_grid_graph_.MTSP_graph_nodes_[Navigation_node_index];
            last_goal_node_ = MTSP_grid_graph_.MTSP_graph_nodes_[to_index];
            last_path_node_index_ = path_node_index;
            //更新机器人状态
            robot_statu_ = RobotStatus::Far_planner;
            have_Navigation_node_ = true;
            Request_breakpoint_count_ = 0;
            last_far_planning_on_local_range_ = false;
            return global_path;
          }
        }

        //目标点在局部范围内   误点  判定
        // bool is_error_point=check_error_point(viewpoint_manager);
        if (last_far_planning_on_local_range_ == false)
        {
          far_breakpoint_coint_++;
          std::cout << "far_breakpoint_coint_  " << far_breakpoint_coint_ << std::endl;
        }
        //目标点在局部范围内  就直接将目标点做为导航点
        exploration_path_ns::Node node_goal(goal_position_, exploration_path_ns::NodeType::GLOBAL_VIEWPOINT);  //目标点
        node_goal.global_subspace_index_ = GetCellInd(goal_position_.x, goal_position_.y, goal_position_.z);
        global_path.Append(node_goal);
        ordered_cell_indices.push_back(node_goal.global_subspace_index_);
        //更新机器人状态
        robot_statu_ = RobotStatus::Far_planner;
        last_far_planning_on_local_range_ = true;
        have_Navigation_node_ = false;
        Request_breakpoint_count_ = 0;
        return global_path;
      }
    }
  ...
  std::cout << "获得    做MTSP的信息    " << std::endl;
  is_fuse_ = GetMtspGridGraphInformation(
      exploring_cell_indices_world, exploring_cell_positions_world, distance_matrix_world, exploring_cell_indices_MTSP,
      exploring_cell_positions_MTSP, distance_matrix_MTSP, robot_position_id_on_exploring_cell_positions_MTSP,
      MTSP_subgrapher_map_, keypose_graph, positions_fuse_grapher_node_id);
  ...
  {  //成功做 MTSP
    ...
    std::cout << " 初始化  MTSP" << std::endl;
    tsp_solver_ns::TSPSolver tsp_solver(data_model);
    std::cout << " 求解  MTSP" << std::endl;
    tsp_solver.Solve();
    std::cout << " 打印  MTSP" << std::endl;
    tsp_solver.getSolutionNodeIndex(node_index, false);  //
    ordered_cell_indices.clear();
    //更新 变量方便后续处理
    exploring_cell_indices = exploring_cell_indices_MTSP;
  ...
  if (is_fuse_)  //  融合成功走这个路线     选一个做目标点  传出
  {
    ...
    else  //如果使用关键位姿图
    {
      ...
      std::cout << "采取far方式" << std::endl;
      std::vector<int> path_node_index;
      int from_index = positions_fuse_grapher_node_id[robot_ind];
      int to_index = positions_fuse_grapher_node_id[goal_ind];
      path_node_index = GetShortestPath_MTSP_grid_graph_path(from_index, to_index);
      goal_position_ = exploring_cell_positions[goal_ind];
      int Navigation_node_index;
      std::cout << "得到导航点" << std::endl;
      bool use_Navigation_node = true;
      Navigation_node_index =
          GetNavigationIndexFromMtspGridGraph(path_node_index, viewpoint_manager, keypose_graph, use_Navigation_node);
      if (Navigation_node_index != -1 && path_node_index.size() >= 2 && use_Navigation_node)
      {
        //将导航点 当作目标   加入发布
        exploration_path_ns::Node Navigation_node(MTSP_grid_graph_.MTSP_graph_nodes_[Navigation_node_index].position_,
                                                  exploration_path_ns::NodeType::GLOBAL_VIEWPOINT);
        Navigation_node.global_subspace_index_ = MTSP_grid_graph_.MTSP_graph_nodes_[Navigation_node_index].cell_id_;
      ...
      {
        std::cout << "导航点不存在  直接发布  目标点当导航点" << std::endl;
        //加入目标
        global_path.Append(node_goal);
        //补偿加入
        global_path.Append(global_path.nodes_[0]);
...
void GridWorld::UpdateGridWorldCellFromOtherRobots(std::map<int, int>& Update_Grid_World_ID_and_Statu_)
{
  ...
  Update_Grid_World_ID_and_Statu_2 = Update_Grid_World_ID_and_Statu_;
    ...
    std::map<int, geometry_msgs::msg::Point>& other_robot_position_map)
{
  ...
  if (robot_statu_ == RobotStatus::Far_planner)
  {
    ...
    if (last_path.GetLength() > 2)
    {
          ...
          CellStatus::EXPLORING)  //目标点的全局还是探索状态，则选用之前计算的结果
      {
        ...
        if (is_reachable)
        {
          std::cout << "===上一次目标子网格  能够到达===" << std::endl;
  ...
  /****************************不为  导航状态   不存在局部路径     求解 全局的   Mdvrp************************/

  // std::cout << "==不为  导航状态   不存在局部路径     求解 全局的   Mdvrp" << std::endl;
  /****** 获取所有连接的    状态为     探测的单元  索引  *****/
  std::vector<geometry_msgs::msg::Point> exploring_cell_positions;        //探索  单元_本地   位置数据
  ...
  // std::cout << "全部neighbor_cell_indices_all.size()  = " << neighbor_cell_indices_all.size() << std::endl;
  ...
  // std::cout << "探索状态的子网格数         = " << count_exploring_cell << std::endl;
  // std::cout << "可到达的待探索子网格数 =  " << num_world_neighbor_cell_reachable << std::endl;
  ...
  for (int i = 0; i < exploring_cell_positions_world.size(); i++)
  {
    for (int j = 0; j < i; j++)
    {
      ...
      else
      {
        ...
        //  是否使用ikdtree  查找
        // distance_matrix_world[i][j] = static_cast<int>(
        //     10 * merger_graph->GetShortestPathTwoPointIkdtree(exploring_cell_positions_world[i],
        //     exploring_cell_positions_world[j],
        //                                         false, path_tmp, true));
      }
    }
  }
