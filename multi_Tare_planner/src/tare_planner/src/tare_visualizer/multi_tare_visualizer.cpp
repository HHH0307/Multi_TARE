...
TAREVisualizer::TAREVisualizer(rclcpp::Node::SharedPtr nh)
{
  ...
  global_subspaces_marker_ =
      std::make_shared<misc_utils_ns::Marker>(nh, "tare_visualizer/exploring_subspaces", robot_name+"/"+kWorldFrameID);
  local_planning_horizon_marker_ =
      std::make_shared<misc_utils_ns::Marker>(nh, "tare_visualizer/local_planning_horizon", robot_name+"/"+kWorldFrameID);

  uncovered_surface_point_cloud_ = std::make_shared<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
      nh, "tare_visualizer/uncovered_surface_points", robot_name+"/"+kWorldFrameID);
  viewpoint_candidate_cloud_ = std::make_shared<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
      nh, "tare_visualizer/viewpoint_candidates", robot_name+"/"+kWorldFrameID);
  viewpoint_cloud_ =
      std::make_shared<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "tare_visualizer/viewpoints", robot_name+"/"+kWorldFrameID);
...
bool TAREVisualizer::ReadParameters(rclcpp::Node::SharedPtr nh)
{
  ...
  nh->get_parameter("kLocalPlanningHorizonMarkerWidth", kLocalPlanningHorizonMarkerWidth); // 已修改
  ...
  double viewpoint_resolution_y = nh->get_parameter("viewpoint_manager/resolution_y").as_double(); // 已修改
