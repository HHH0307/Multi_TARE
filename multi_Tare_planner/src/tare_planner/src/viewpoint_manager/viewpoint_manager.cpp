...
{
  nh->get_parameter("kUseFrontier", kUseFrontier); // 是否使用前沿点
  ...
  dimension_ = 2; // 2维
  kViewPointNumber = kNumber.x() * kNumber.y() * kNumber.z(); // 视点总数
  kRolloverStepsize = kNumber / 5; // 滚动步长 设置为视点数量的五分之一
...
// 添加 得到  水平最近障碍物距离
double ViewPointManager::GetLidarNearestObstacleDistance(int viewpoint_ind, bool use_array_ind)
{
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
   return viewpoints_[array_ind].GetNearestObstacleDistance();
}
...
int ViewPointManager::GetViewPointCandidate()
{
  ...
  for (int i = 0; i < vp_.kViewPointNumber; i++)
  {
    ...
    if( robot_cell_statu== grid_world_ns::CellStatus::COVERED_BY_OTHERS ||robot_statu_==grid_world_ns::RobotStatus::Request ||robot_statu_==grid_world_ns::RobotStatus::Far_planner||last_isglobal_tsp_==true )
    {
      //  保守     的获取候选点
         if (!ViewPointInCollision(i) &&ViewPointInCurrentFrameLineOfSight(i) && ViewPointConnected(i))  //  修改，必须在当前视线内，不在障碍物里，视点是连通的
        {
        SetViewPointCandidate(i, true);  //设置为候选视点
        candidate_indices_.push_back(i); //加入候选视点集合
        geometry_msgs::msg::Point viewpoint_position = GetViewPointPosition(i);
        pcl::PointXYZI point;
        point.x = viewpoint_position.x;
        point.y = viewpoint_position.y;
        point.z = viewpoint_position.z;
        viewpoint_candidate_cloud_->points.push_back(point);   //得到候选视点  点云
      }
    }
    else{
      //    激进的获取可通行点
      //测试     ViewPointConnected(i)条件  已经有了   ViewPointInLineOfSight(i)
     // if (!ViewPointInCollision(i) && ViewPointInLineOfSight(i) && ViewPointConnected(i))    //不在障碍物里，在视线范围内，视点是连通的
      if (!ViewPointInCollision(i) && ViewPointConnected(i))    //不在障碍物里，在视线范围内，视点是连通的
        {
        SetViewPointCandidate(i, true);  //设置为候选视点
        candidate_indices_.push_back(i); //加入候选视点集合
        geometry_msgs::msg::Point viewpoint_position = GetViewPointPosition(i);
