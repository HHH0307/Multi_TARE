                   ...
                   bool get_path, std::vector<int>& path_indices)
{
  ...
  double INF = 99999.0; // 2025-10-30 修改为更大的值，防止计算不出路径时返回0距离
  typedef std::pair<double, int> iPair;
  double shortest_dist = 99999.0; // 计算不出距离的时候用最大值代替 原来为 0
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//新增 有关  fringe search 的相关函数
fringe_search_node::fringe_search_node() {
  this->f = 0;
  this->open = false;
...
void fringe_search_node::expand() {
  this->is_closed = true;
  this->open = false;
}
void fringe_search_node::relax(double g, double h, fringe_search_node* whence) {
  this->f = g + h;
  this->g = g;
  this->h = h;
  this->whence = whence;
...
/*
函数  创建    FringeSearchGraph
输入
vector<vector<int>> graph
vector<vector<double>> node_dist
vector<geometry_msgs::msg::Point> node_positions
输出
vector<fringe_search_node> Nodes
*/
void FringeSearchGraph(const std::vector<std::vector<int>>& graph,
const std::vector<std::vector<double>>& node_dist,
const std::vector<geometry_msgs::msg::Point>& node_positions,
std::vector<fringe_search_node>& Nodes)
{
    for(int i=0;i<node_positions.size();i++)
    {
        fringe_search_node temp_node;
        temp_node.node_index=i;
        temp_node.posetion=node_positions[i];
        temp_node.neighbors=graph[i];
        temp_node.neighbors_distance=node_dist[i];
        Nodes.push_back(temp_node);
    }
}
double GetH(fringe_search_node* start,fringe_search_node* goal)
{
  double dis=-1;
  if(start->h==-1)//第一次计算
  {
    dis=PointXYDist(start->posetion,goal->posetion);
  }
  else//计算过  减少一次计算，直接赋值。
  {
    dis=start->h;
  }
  return dis;
}
double GetCost(fringe_search_node* node1,fringe_search_node* node2)
{
  return PointXYDist(node1->posetion,node2->posetion);
}
void reconstruct_path(fringe_search_node* start, fringe_search_node* current, std::vector<int>& path_indices ) 
{
  path_indices.push_back(current->node_index);
  while (current != start)
  {
...
fringe_search
*/
void fringe_search(std::vector<fringe_search_node>& Nodes,int from_idx, int to_idx, bool get_path,std::vector<int>& path_indices) {
 
 //std::cout<<"进入函数"<<std::endl;
  fringe_search_node* start=&Nodes[from_idx];
  fringe_search_node* goal=&Nodes[to_idx];
  static std::list<fringe_search_node*> Fringe; //边缘  结点
  Fringe.push_back(start); //边缘  结点 加入起点
  start->open = true; //设置 起点  在  open  list   中（不需要维护排序，只需要是否在里面)
  double h=GetH(start, goal);
  start->relax(0, h, NULL); //设置  g 和  f ;  f =g+h    0+h(start,goal);    起点  没有前一个点了  所以是 NULL
 ...
 //std::cout<<"fringe search 初始化成功"<<std::endl;
  while (!found && !Fringe.empty()) {
    ...
    for (auto ff = Fringe.begin(); ff != Fringe.end();) {  //从  Fringe  的第一个到  最后一个
      ...
      if (expand_me->f > f_limit) {   //当前权值 大于  f_limit  f限制值
        if (expand_me->f < next_f_limit)  //  当前权值  小于 next_f_limit
        {
          next_f_limit = expand_me->f; // track smallest next depth  跟踪下一个最小深度 
        }
        ++ ff;//  查询下一个
     ...
     //++ stats.nodes_expanded;  //记录状态的 节点扩展
      ...
      for (auto& add_me_index: expand_me->neighbors) {  //当前点的  邻接点
        ...
        if (!add_me->open) {// add_me 不在open list上  (一般第一次)
          ...
          add_me->relax(g, GetH(add_me, goal), expand_me);//更新 g,f 的值
          //  在  ff  之后   insertion_point
          auto insertion_point = next(ff);
          //  使用  insert(pos,elem)则是在  insertion_point之前，即  ff 的后面插入  add_me
          //iterator  =    .insert(pos,elem) 在迭代器 pos 指定的位置之前插入一个新元素 elem，并返回表示新插入元素位置的迭代器。
          add_me->fringe_index = Fringe.insert(insertion_point, add_me);//把点 add_me 加入到
        }
        else if (g < add_me->g) {  //新找到的路径 g  小于之前的路径 g   则更新
          add_me->relax(g, GetH(add_me, goal), expand_me);//更新 与起点的新 g ， h不变 还是之前的 h
          //  在  ff  之后  点  insertion_point
          auto insertion_point = next(ff);
          if (*insertion_point != add_me) {
            Fringe.erase(add_me->fringe_index);
            add_me->fringe_index = Fringe.insert(insertion_point, add_me);//重新在 ff 后面 加入 add_me
          }
        }
      }
      ff = Fringe.erase(ff);//去掉 ff
    }
    // Increase the depth and scan the fringe again 增加深度并再次扫描边缘
    f_limit = next_f_limit;
  }
   //std::cout<<"fringe search 查询结束"<<std::endl;

  // Stats collection & cleanup
 //stats.open_list_size += Fringe.size();
 if(get_path &&found)
 {
   reconstruct_path(start, goal, path_indices);
   std::reverse(path_indices.begin(), path_indices.end());
 }
 //只做一次，不保留。
