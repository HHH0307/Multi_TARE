...
// 新增改动
int LocalCoveragePlanner::GetBoundaryViewpointIndex_pro(const exploration_path_ns::ExplorationPath& historical_path)
{
  int boundary_viewpoint_index = robot_viewpoint_ind_;  // 机器人当前视点初始化边界点
  return boundary_viewpoint_index;
}
...
//新增改动
void LocalCoveragePlanner::GetBoundaryViewpointIndices_pro(exploration_path_ns::ExplorationPath historical_path)
{
  start_viewpoint_ind_ =
      GetBoundaryViewpointIndex_pro(historical_path);  //得到局部规划框的   视点边界   起点  和终点     设置为一个点
...
//修改  新增的  得到导航视点  ID
void LocalCoveragePlanner::GetNavigationViewPointIndices_pro(exploration_path_ns::ExplorationPath historical_path,
                                                             std::vector<int>& navigation_viewpoint_indices)
{
...
//添加  修改排序  得到候选点
void LocalCoveragePlanner::EnqueueViewpointCandidates_target_point(
    std::vector<std::pair<int, int>>& cover_point_queue, std::vector<std::pair<int, int>>& frontier_queue,
    const std::vector<bool>& covered_point_list, const std::vector<bool>& covered_frontier_point_list,
    const std::vector<int>& selected_viewpoint_array_indices)
{
  for (const auto& viewpoint_index : viewpoint_manager_->GetViewPointCandidateIndices())  //对于每个候选点
  {
    if (!viewpoint_manager_->ViewPointVisited(viewpoint_index))  //就是要在  访问过的点上（已探索的单元内）
                                                                 //寻求路径到达其他目标点
    {
      continue;
    }
    ...
    if (std::find(selected_viewpoint_array_indices.begin(), selected_viewpoint_array_indices.end(),
                  viewpoint_array_index) != selected_viewpoint_array_indices.end())  //在预选中点上    跳过
    {
      continue;
    }
    int covered_point_num = viewpoint_manager_->GetViewPointCoveredPointNum(covered_point_list, viewpoint_array_index,
                                                                            true);  //当前点  覆盖   未覆盖点  数量
    if (covered_point_num >= parameters_.kMinAddPointNum)
    {
      cover_point_queue.emplace_back(covered_point_num, viewpoint_index);  //由  <数量,viewpoint_index  ID>
    }
    else if (use_frontier_)
    {
      ...
      if (covered_frontier_point_num >= parameters_.kMinAddFrontierPointNum)
      {
        frontier_queue.emplace_back(covered_frontier_point_num, viewpoint_index);  //由  <数量,viewpoint_index  ID>
      }
    }
  }

  // Sort the queue
  std::sort(cover_point_queue.begin(), cover_point_queue.end(), SortPairInRev);  //排序
  if (use_frontier_)
  {
    std::sort(frontier_queue.begin(), frontier_queue.end(), SortPairInRev);  //排序
  }
}
...
void LocalCoveragePlanner::SelectViewPoint(
  ...
  std::mt19937 gen(rd()); // std::mt19937是C++11加入的新特性，是一种高效的随机数生成算法。它的用法和rand()相似，但是具有速度快，周期长的特点。它的名字就来自于周期长度2^19937-1
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//添加函数
exploration_path_ns::ExplorationPath LocalCoveragePlanner::SolveTSP_without_StartAndEnd(
    const std::vector<int>& selected_viewpoint_indices, std::vector<int>& ordered_viewpoint_indices)

{
  // nav_msgs::msgs::Path tsp_path;
  exploration_path_ns::ExplorationPath tsp_path;
  ...
  std::cout << "导航点个数  " << navigation_viewpoint_num_ << std::endl;
  std::cout << "被选中点个数  " << selected_viewpoint_indices.size() << std::endl;
  if (navigation_viewpoint_num_ > selected_viewpoint_indices.size())
  {
    std::cout << "导航点个数大于   被选择点个数 不求 局部路径" << std::endl;
  ...
  for (int i = 0; i < selected_viewpoint_indices.size(); i++)
  {
    if (selected_viewpoint_indices[i] == robot_viewpoint_ind_)  //找到当前机器人位置点
    {
      robot_ind = i;
    }
    if (selected_viewpoint_indices[i] == lookahead_viewpoint_ind_)  //找到前一个目标点
    {
      lookahead_ind = i;
    }
  }
  ...
  for (int i = 0; i < selected_viewpoint_indices.size(); i++)
  {
    ...
    else if (selected_viewpoint_indices[i] == lookahead_viewpoint_ind_)  //找到前一个目标点
    {
      lookahead_ind = i;
      k++;
    }
  }
  if (k == selected_viewpoint_indices.size())
  {
    local_coverage_complete_ = true;
    return tsp_path;
  }

  bool has_robot_lookahead_dummy = robot_ind != lookahead_ind;  //机器人位置和目标点是否一致

  // Get distance matrix
  int node_size;

  if (has_robot_lookahead_dummy)  //其中一个一致
  {
    node_size = selected_viewpoint_indices.size() + 1;  //结点数加1
  }
  else
  {
    node_size = selected_viewpoint_indices.size();  //其余正常
  }
  misc_utils_ns::Timer find_path_timer("find path");
  find_path_timer.Start();
  std::vector<std::vector<int>> distance_matrix(node_size, std::vector<int>(node_size, 0));
  ...
  for (int i = 0; i < selected_viewpoint_indices.size(); i++)
  {
    ...
    {
      int to_ind = selected_viewpoint_indices[j];
      nav_msgs::msg::Path path = viewpoint_manager_->GetViewPointShortestPath(from_ind, to_ind);  //计算两个点的最短     path
      double path_length = misc_utils_ns::GetPathLength(path);
      distance_matrix[i][j] = static_cast<int>(10 * path_length);
    }
  }
  ...
  // Add a dummy node to connect the start and end nodes   添加虚拟节点以连接开始节点和结束节点

  if (has_robot_lookahead_dummy)
  {
    int end_node_ind = node_size - 1;
    for (int i = 0; i < selected_viewpoint_indices.size(); i++)
    {
  ...
  if (path_index.size() > 1)
  {
    ...
    for (int i = 0; i < path_index.size() - 1; i++)
    {
      ...
      {
        next_node.type_ = exploration_path_ns::NodeType::LOOKAHEAD_POINT;  // 前一次规划的的目标点
      }
      tsp_path.Append(next_node);
    }
  }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//修改SolveLocalCoverageProblem_pro
exploration_path_ns::ExplorationPath LocalCoveragePlanner::SolveLocalCoverageProblem_pro(const exploration_path_ns::ExplorationPath& historical_path,
                                                    int uncovered_point_num, int uncovered_frontier_point_num)
{
  ...
  if (!queue.empty() && queue[0].first > parameters_.kMinAddPointNum)  //排序不未空,且第一个点数量大于最小添加点.
  {
    ...
    for (int itr = 0; itr < parameters_.kLocalPathOptimizationItrMax; itr++)  //局部规划优化 个数  10
    {
      ...
      // Select from the queue     选出备用的候选点
      misc_utils_ns::Timer select_viewpoint_timer("select viewpoints");
      select_viewpoint_timer.Start();
      SelectViewPoint(queue, covered, selected_viewpoint_indices_itr, false);  //核心 选点 并更新奖励
      SelectViewPointFromFrontierQueue(frontier_queue, frontier_covered, selected_viewpoint_indices_itr);

      // Add viewpoints from last planning cycle   加入之前预选点
      for (const auto& ind : reused_viewpoint_indices)
      {
        selected_viewpoint_indices_itr.push_back(ind);
      }
      // Add viewpoints for navigation    导航点加在最后   用于识别
      for (const auto& ind : navigation_viewpoint_indices)
      {
        selected_viewpoint_indices_itr.push_back(ind);
      }
  ...
  else
  {
    ...
    // Add viewpoints for navigation   增加视点  用于导航
    for (const auto& ind : navigation_viewpoint_indices)
    {
      selected_viewpoint_indices_itr.push_back(ind);
    }

    misc_utils_ns::UniquifyIntVector(selected_viewpoint_indices_itr);  //剔除重复
  ...
  for (const auto& viewpoint_index : last_selected_viewpoint_indices_)  //上一次被选中的点
  {
