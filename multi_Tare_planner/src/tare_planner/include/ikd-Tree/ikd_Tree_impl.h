...
void KD_TREE<PointType>::start_thread(){
    ...
    pthread_create(&rebuild_thread, NULL, multi_thread_ptr, (void*) this);  //创建  重建线程
...
void KD_TREE<PointType>::multi_thread_rebuild(){
    ...
    while (!terminated){
        ...
        pthread_mutex_lock(&termination_flag_mutex_lock);
        terminated = termination_flag;//结束标志
        pthread_mutex_unlock(&termination_flag_mutex_lock);
        usleep(100); //  程序挂起  100us
    }
    ...
    vector<float> & Point_Distance, double max_dist)
{
    ...
    int k_found = min(k_nearest,int(q.size()));  //实际搜索到的个数和  k_nearest  取较小的
