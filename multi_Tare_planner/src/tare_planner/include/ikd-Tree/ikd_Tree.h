...
// wgh 定义box（一个box由两个边界点定义，最小和最大的边界点定义）。
struct BoxPointType{
    float vertex_min[3];
    float vertex_max[3];
};
...
class MANUAL_Q{
    private:
        ...
        T q[Q_LEN]; // 存储 T 类型的数组 最大数量  Q_LEN =   
        bool is_empty;
    public:
        void pop();
        T front();
...
class KD_TREE {
  ...
  public:
    ...
    // 重要 knn  搜索   搜索中心  ， k 个数  ,搜索结果 ， 按顺序对应的距离  ，最大搜索距离
    ...
    int Add_Points(PointVector & PointToAdd, bool downsample_on=false);  // 当前的应用场景不需要  删除点
