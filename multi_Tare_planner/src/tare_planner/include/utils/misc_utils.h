...
//新增  有关fringe search 
//定义   fringe search  结点
class fringe_search_node
{
public:
  int node_index; //结点编号
  std::vector<int> neighbors;   //当前  node 的邻接
  std::vector<double> neighbors_distance;//当前  node 的邻接 点 距离
  geometry_msgs::msg::Point posetion;  //当前  node  的坐标
  // pathfinding variables  寻路变量
  double g;  //当前结点  到开始结点的    最小  距离     可以更新改动
  double h;  //当前结点  到目标结点的    预估   距离
  double f; //     f =h+g  综合成本
  fringe_search_node * whence;    // for reconstructing paths  用于重建路径     当前结点的  前一个结点
  std::list<fringe_search_node*>::iterator fringe_index; // location in the fringe (fringe search)用于边缘搜索  记录边缘中的位置
  bool is_closed;
  bool open;                          // whether this node is on an open list   此结点是否在 open list
  fringe_search_node();
  void expand();
  void relax(double g, double h, fringe_search_node* whence);
  inline bool closed() { return this->is_closed; }
};
...
void fringe_search(std::vector<fringe_search_node>& Nodes, int from_idx, int to_idx, bool get_path,std::vector<int>& path_index);
...
void reconstruct_path(fringe_search_node* start, 
                                                fringe_search_node* current,
