...
public:
  ...
  explicit LiDARModel(double px = 0.0, double py = 0.0, double pz = 0.0, double rw = 1.0, double rx = 0.0, //explicit  避免隐式转换
    ...
    for (int n = -horizontal_neighbor_num; n <= horizontal_neighbor_num; n++)
    {
      ...
      for (int m = -vertical_neighbor_num; m <= vertical_neighbor_num; m++)
      {
        ...
        if (isZero(previous_distance_to_point) || distance_to_point < previous_distance_to_point || reset_[ind])
        {
          ...
          // 新增
          if(dz>-0.3 && dz<0.3 &&Nearest_horizontalobstacle_distance_>distance_to_point)
          {
            Nearest_horizontalobstacle_distance_ = distance_to_point;
          }
          reset_[ind] = false;
        }
      }
  ...
  // 添加  
  float GetNearestObstacleDistance()
  {
    return Nearest_horizontalobstacle_distance_;
  }
...
private:
  ...
  // 新增 当前最近障碍物距离。    从    covered_voxel_  取最小值
  float Nearest_horizontalobstacle_distance_;
};
}  // namespace lidar_model_ns